...
export function useBarChart(datasets: ChartDataset[], options: BarChartOptions = {}) {
  ...
  const calculateBarDimensions = useCallback((
    ...
    const allValues = datasets.flatMap(dataset => 
      dataset.data?.map(d => d.value).filter(v => typeof v === 'number') || []
    );
      ...
      dataset.data?.forEach((point, categoryIndex) => {
        ...
        if (horizontal) {
          ...
          if (options.stacked && datasetIndex > 0) {
            barX = padding.left + ((stackedPositions[categoryIndex] - minValue) / valueRange) * innerWidth;
          ...
          if (options.stacked && datasetIndex > 0) {
            const stackedHeight = ((stackedPositions[categoryIndex] - minValue) / valueRange) * innerHeight;
        ...
        if (options.maxBarWidth && (horizontal ? barHeight : barWidth) > options.maxBarWidth) {
          if (horizontal) {
            ...
            barY = padding.top + categoryIndex * (innerHeight / numCategories) + 
    ...
    return bars;
  }, [options.stacked, options.groupPadding, options.barPadding, options.minBarHeight, options.maxBarWidth]);
  ...
  // Bar hover handlers
  const handleBarHover = useCallback((datasetIndex: number, pointIndex: number, x: number, y: number) => {
  ...
  const generateGradients = useCallback((datasets: ChartDataset[]) => {
  ...
  const getDataLabelPosition = useCallback((bar: BarDimensions, horizontal = false) => {
