import { BreadcrumbItemData } from '../../components/Breadcrumb/Breadcrumb';
...
export function useBreadcrumb(initialOptions?: Partial<BreadcrumbOptions>) {
   ...
   * @returns Class string
   */
  const generateItemClass = (item: BreadcrumbItemData, isLast: boolean): string => {
   ...
   */
  const isItemLink = (item: BreadcrumbItemData, isLast: boolean): boolean => {
   ...
   */
  const parseItemsFromJson = (jsonString: string): BreadcrumbItemData[] => {
    try {
      return JSON.parse(jsonString) as BreadcrumbItemData[];
    } catch (error) {
