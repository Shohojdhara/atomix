    ...
    (
      ...
      const renderChart = () => {
        ...
        const barWidth = horizontal 
          ? (scales.height - scales.padding.top - scales.padding.bottom) / processedData.length * 0.8
          : (scales.width - scales.padding.left - scales.padding.right) / processedData.length * 0.8;
          ...
          <svg width="100%" height="100%" viewBox={`0 0 ${scales.width} ${scales.height}`}>
            ...
            {processedData.map((point, index) => {
              ...
              if (horizontal) {
                ...
                const barY = scales.padding.top + index * (scales.height - scales.padding.top - scales.padding.bottom) / processedData.length + (barWidth * 0.1);
                ...
                return (
                  <g key={index}>
                    <rect
                      x={barX}
                      y={barY}
                      width={Math.max(0, (point.value / Math.max(...processedData.map(d => d.value))) * (scales.width - scales.padding.left - scales.padding.right))}
                      height={barHeight}
                      ...
                      onMouseEnter={(e) => {
                        if (interactive) {
                          ...
                          const clientX = rect ? rect.left + (barX + (point.value / Math.max(...processedData.map(d => d.value))) * (scales.width - scales.padding.left - scales.padding.right) / 2) : e.clientX;
                    ...
                    {showValues && (
                      <text
                        x={barX + (point.value / Math.max(...processedData.map(d => d.value))) * (scales.width - scales.padding.left - scales.padding.right) + 5}
                        y={barY + barHeight / 2}
            ...
            {processedData.map((point, index) => (
              <text
                ...
                y={horizontal ? scales.padding.top + index * (scales.height - scales.padding.top - scales.padding.bottom) / processedData.length + barWidth / 2 : scales.height - 10}
