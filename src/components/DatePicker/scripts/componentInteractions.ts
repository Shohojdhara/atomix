 ...
 */
export function handleDateGridKeyboardNavigation(event: KeyboardEvent, container: HTMLElement): void {
  if (!container) return;
  
  const focusableItems = container.querySelectorAll<HTMLElement>('[role="gridcell"]:not([disabled])');
  ...
  switch (event.key) {
    ...
    case 'ArrowDown':
      ...
      const rowSize = container.classList.contains('c-datepicker__days') ? 7 : 
                      container.classList.contains('c-datepicker__months') ? 3 : 4;
    ...
    case 'ArrowUp':
      ...
      const rowSizeUp = container.classList.contains('c-datepicker__days') ? 7 : 
                        container.classList.contains('c-datepicker__months') ? 3 : 4;
    ...
    case 'PageUp':
      ...
      const prevMonthButton = container.closest('.c-datepicker__calendar')?.querySelector<HTMLElement>('.c-datepicker__nav-button--prev-month');
      if (prevMonthButton) {
    ...
    case 'PageDown':
      event.preventDefault();
      // Trigger month navigation in day view
      const nextMonthButton = container.closest('.c-datepicker__calendar')?.querySelector<HTMLElement>('.c-datepicker__nav-button--next-month');
      if (nextMonthButton) {
...
export function enhanceWithKeyboardNavigation(datepicker: DatePicker): void {
  ...
  calendar.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    if (target.closest('.c-datepicker__days')) {
      handleDateGridKeyboardNavigation(e, calendar.querySelector('.c-datepicker__days') as HTMLElement);
    } else if (target.closest('.c-datepicker__months')) {
      handleDateGridKeyboardNavigation(e, calendar.querySelector('.c-datepicker__months') as HTMLElement);
    } else if (target.closest('.c-datepicker__years')) {
      handleDateGridKeyboardNavigation(e, calendar.querySelector('.c-datepicker__years') as HTMLElement);
    }
